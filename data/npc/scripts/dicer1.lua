...
local function rollDice(roll, cc_count, pc_count, table_left_pos, table_middle_pos, npc)
        ...
        if ((total_g) <= 500000 and (total_g) >= 30000) then
                ...
                if (roll == 1 and random_rollval <= 3) then
                        ...
                        addEvent(doCreatureSay, 500, npc, "GANASTE =P!", TALKTYPE_SAY, false, 0)
                else
                        ...
                        addEvent(doCreatureSay, 500, npc, "PERDISTE JAJAJAJ.", TALKTYPE_SAY, false, 0)
                end
                ...
                doCreatureSay(npc, "Minimo para jugar 30K y el maximo es 500k.", TALKTYPE_SAY, false, 0)
        end
        return true
end
 ...
 function onThink()
	...
	local npcpos = {x = 32342, y = 32226, z = 7}
	
	local creature = Tile(32342, 32228, 7):getTopCreature()
	if creature and creature:isPlayer() then
	selfTurn(SOUTH)
	--doCreatureSay(npc, "HOLA |PLAYERNAME|, Do your bets.", TALKTYPE_SAY, false, 0)
    else
    selfTurn(EAST)
	--doCreatureSay(npc, "NOS VEMOS XD |PLAYERNAME|!.", TALKTYPE_SAY, false, 0)
	end
...
function creatureSayCallback(cid, type, msg)
        ...
        local table_left_pos = {x = 32341, y = 32226, z = 7} -- 32341, 32224, 7
        local table_middle_pos = {x = 32341, y = 32227, z = 7} -- 32341, 32225, 7
        ...
        posplayer = {x = 32342, y = 32228, z = 7} -- 323439, 32225, 7
        ...
        if isInArray({"H", "HIGH", "high", "h"}, msg) then
                ...
                elseif  isInArray({"L", "LOW", "l", "low"}, msg) then
						...
						elseif  isInArray({"juega", "juego"}, msg) then
doCreatureSay(npc, "Hola, vamos a jugar? low ou high? Di Low=1,2,3 o High=4,5,6.", TALKTYPE_SAY, false, 0)  
