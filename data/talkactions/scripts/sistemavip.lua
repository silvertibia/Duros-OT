...
local storageplayer = getPlayerStorageValue(cid, 55778)
local timenow = os.time()

if getPlayerStorageValue(cid, 55778) - os.time() <= 0 then
...
setPlayerStorageValue(cid, 55778, time)
local quantity = math.floor((getPlayerStorageValue(cid, 55778) - timenow)/(24 * 60 * 60))
...
local quantity = math.floor((getPlayerStorageValue(player, 55778) - timenow)/(24 * 60 * 60))
...
local storageplayer = getPlayerStorageValue(player, 55778)
local timenow = os.time()
local time = storageplayer <= 0 and (timenow + daysvalue) or (storageplayer + daysvalue)

if string.find(tostring(getCreatureName(pid)),"[[Vip]]") then
doPlayerSendTextMessage(player, MESSAGE_INFO_DESCR, "Foram adicionados "..days.." dias de VIP no seu character.")
setPlayerStorageValue(player, 55778, time)
local quantity = math.floor((getPlayerStorageValue(player,55778) - timenow)/(3600*24))
...
if(getPlayerStorageValue(C.uid,55778) < C.time)then
doPlayerSendTextMessage(cid,dec,'O jogador '..t[1]..' não possui '..C.days..' dias de vip.')
else
doPlayerSendTextMessage(cid,dec,'Você removeu '..C.days..' dias de vip do player '..t[1]..'.')
setPlayerStorageValue(C.uid,55778,getPlayerStorageValue(C.uid,55778)-C.time)
end
